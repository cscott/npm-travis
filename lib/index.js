...
var getRepoName = function() {
	return Promise.resolve(program.args[0]).then(function(reponame) {
		...
		return p.execFile('git', ['remote','show','github']).then(function(out){
...
var deleteBranch = function(repo, branchname) {
	return ensureRemote(repo).then(function() {
		return p.spawn('git', ['push', 'github', ':'+branchname], {
			timeout: 60*1000,
	...
	var initState = function() {
		return Promise.all([
			...
			console.log(
				...
				state.build.number,
				'(',
				'https://'+state.config.host+'/'+state.repo+'/builds/'+state.build.id,
				')'
			);
			return state;
		});
	...
	var watchJob = function(job_num) {
		return Promise.resolve().then(function() {
			...
			console.log(
				...
				'(',
				'https://'+state.config.host+'/'+state.repo+'/jobs/'+state.job_id,
				')'
			);
			// check job #
			if (!state.job_id) {
