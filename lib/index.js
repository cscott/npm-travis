...
var ensureBranch = function(repo, branchname) {
	return ensureRemote(repo).then(function() {
		return p.spawn('git', ['push', program.remote, 'HEAD:'+branchname], CHILD_IGNORE);
	});
};
...
var getTravisRepo = function(reponame) {
	return apiRequest('/repos/'+reponame).get('repo');
};
...
var doit = function() {
	...
	var initState = function() {
		return Promise.all([
			...
			return getTravisBuild(state.repo.id, state.branch, false).catch(function(e) {
				...
				return ensureBranch(state.reponame, state.branch).then(function() {
					return getTravisBuild(state.repo.id, state.branch, true);
				});
			});
		}).then(function(build) {
	...
	var cleanup = function() {
		return Promise.resolve().then(function() {
			pusher.disconnect();
			return getTravisBuild(state.repo.id, state.branch, false);
		}).then(function(build) {
			state.build = build;
